...
//echo "<pre>";print_r("select tbljobs.* from tbljobs  LEFT JOIN tbljobcategories ON tbljobs.job_id=tbljobcategories.job_id LEFT JOIN tbljobstatus ON tbljobs.job_id=tbljobstatus.job_id where  tbljobs.status=1 and tbljobs.job_status=1 and tbljobs.start_date >='".date("Y-m-d h:i:s")."' ".$catjoin."  and tbljobs.payment_status=0 ".$user_cond." ".$searchtxt." group by tbljobs.job_id ".$orderby." LIMIT $start,$pagelimit");exit;
	...
	{	
		...
		$data.='<li>
                    					...
                    					<div class="stj_jl_rgt">
                    						...
                    						<a class="a_pyb" href="biddetails.php?job_id='.$rows["job_id"].'&flag=pending">Details</a>
