...
<title>Search Candidate - Confirmed - SECURE THAT JOB</title>
		...
		if($('#last_page').val()==0)
		 {
			var activesort=$("ul.sort_ul li").find("a.active").attr("data-sortby");
			 var page=1;
			document.cookie="requested="+page;
		load_more(getCookie('requested'),category,activesort);
		 }
...
$(window).scroll(function(){
	...
	$('.stj_loader').hide();
		...
		var pages=getCookie('requested');
		    pages++;
		 var pagenumber=document.cookie="requested="+pages;
       ...
       var activesort=$("ul.sort_ul li").find("a.active").attr("data-sortby");		
        load_more(getCookie('requested'),category,activesort);
        $('#current_page').val(getCookie('requested'));		
		    }
		 }
		 else
		 {
...
function load_more(page,cat,sort){
	...
	var confirmedby = '<?php echo $_SESSION['user_id'] ?>';
      $.ajax(
            {
                url: 'get_new_request.php?page='+page+'&category='+cat+'&sort='+sort+searchval+'&confirmedby='+confirmedby,
                type: "get",
                datatype: "html",
                beforeSend: function()
                {
