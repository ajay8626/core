...
for($i=0; $i<count($_FILES['file']['name']); $i++){
	...
	/* Location */
	$location = "admin/user/company_certificates/".$_FILES['file']['name'][$i];
	...
	if(move_uploaded_file($_FILES['file']['tmp_name'][$i],$location)){
		...
		<?php if($mimeType != "pdf"){ ?>
		<span id="uploaded_company_certi_<?php echo $i;?>">
			<a class="fancybox" rel="group" href="<?php echo SITE_URL.''.$location; ?>">
				<img src="<?php echo SITE_URL.''.$location; ?>" style="width:100px;" alt="">
			</a>
			<input type="button" name="remove_certi" class="btn btn-srch-pc remove_certi" value="Remove" onclick="javascript:return removeCompanyCertiUpload(<?php echo $i;?>);">
		</span>
		<?php }else{ ?>
			<span id="uploaded_company_certi_<?php echo $i;?>">
