...
<!-- All Calendar Start -->
<?php
$featurejob = mysql_query("SELECT  tbljobs.job_name AS title, tbljobs.start_date AS start, CASE WHEN 1=1 THEN '#CF2027' END AS color, CASE WHEN 1=1 THEN tbljobs.job_id END AS url FROM tbljobs 
LEFT JOIN tbljobcategories ON tbljobs.job_id=tbljobcategories.job_id 
LEFT JOIN tbljobstatus ON tbljobs.job_id=tbljobstatus.job_id 
LEFT JOIN  tbljobsapplied ON tbljobs.job_id=tbljobsapplied.job_id 
where tbljobs.job_status in (1,2) 
AND tbljobs.payment_status in (-1,1) 
AND  tbljobs.status=1 
AND tbljobs.start_date >='".date("Y-m-d h:i:s")."' 
AND tbljobsapplied.user_id=21 
and tbljobsapplied.is_winner = 1");
