   ...
   while($rows=mysql_fetch_array($featurejob))
   {
		...
		$data.='<li>
				...
				<div class="stj_jl_rgt">
					...
					<a class="a_pybb" href="place-bid.php?job_id='.$rows["job_id"].'">Place your bid</a>
					<a class="a_pyba" onclick="javascript:return applyForJob('.$rows["job_id"].', '.$user_id.', '.$rows["price"].');">Apply Now</a>
