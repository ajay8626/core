...
$select_query = mysql_query("SELECT ct.*,s.name as statename , c.name as countryname FROM `tblcities` as ct LEFT JOIN `tblstates` AS s ON s.id = ct.state_id LEFT JOIN `tblcountries` AS c ON c.id = s.country_id order by statename asc");	
