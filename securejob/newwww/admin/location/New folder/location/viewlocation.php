...
// delete record
	...
	if($delRqst) {
		foreach ($_POST["del"] as $deletAdminID) {	
			if(is_numeric($deletAdminID) && $deletAdminID > 0 ){	
				$where = "location_id  = {$deletAdminID}";
				if($db->Delete("tbllocation",$where)){
					$_SESSION['mt'] = "success";
					$_SESSION['me'] = "location deleted successfully.";
				}else{
					$_SESSION['mt'] = "error";
		...
		header('Location:main.php?pg=viewlocation&delete');
		exit;
	...
	{
		$select_query = mysql_query("select * from tbllocation where location_id=".$_REQUEST['id']."");
		$rows=mysql_num_rows($select_query);
		if($rows > 0)
		{
			$delete=mysql_query("delete from tbllocation where location_id=".$_REQUEST['id']."");
			$res=mysql_fetch_array($select_query);
			$text=$res['name'];
			$_SESSION['mt'] = "success";
			$_SESSION['me'] = $text." location has been deleted successfully.";
		...
		{
			$_SESSION['mt'] = "error";
			$_SESSION['me'] = "Error while delete Location. Please try again.";
			header('Location:main.php?pg=viewlocation');
		    exit;		
		}
	}
$select_query = mysql_query("select * from tbllocation  ORDER By `locationname` ASC");	
?>
    ...
    <!-- Content Header (Page header) -->
		<section class="content-header">
		  ...
		  <ol class="breadcrumb">
			...
			<li><a href=""><i class="fa fa-location-arrow"></i>Locations</a></li>
					 ...
					 <form method="post" name="form">	
					  ...
					  <input type="hidden" name="delId" id = "delId" value="" />	
						...
						if(mysql_num_rows($select_query)){
							...
							echo "<tr>
										<td><input type='checkbox' name='del[]' value='".$row["location_id"]."'></td>
										<td>".$row["locationname"]."</td>"; ?>
																				
										 <td><a class='btn btn-primary' href='main.php?pg=modlocation&id=<?=$row["location_id"]?>'>Modify</a>
										<a class='btn btn-warning' onclick="return confirm('Are you sure want to delete this state?')" href='main.php?pg=viewlocation&id=<?=$row["location_id"]?>' title="are you sure want to delete this state?">Delete</a></td>
									 </tr>
						<?php			 
						...
						<div >
							...
							<button type="button" onclick="location.href='main.php?pg=addlocation'"  name="submit_me"  class="btn btn-primary">Add</button>
						</div>
					  </form>
					</div>
					<!-- /.box-body -->
...
function changeContent(id,val){
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "phpajax/LocationActiveInactive.php", 
		data: data,
		beforeSend: function() {
