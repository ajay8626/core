...
$Query= "SELECT * from tblrating order by rating_name asc";
...
if($delRqst && !isset($_REQUEST['del'])) {
	...
	if(is_numeric($delId) && $delId > 0 ){	
		...
		$where = " rating_id = {$delId} ";
		$db->Delete("tblrating",$where);		
	}
	$_SESSION['mt'] = "success";
	$_SESSION['me'] = "Rating deleted successfully";
...
<div class="content-wrapper">
	...
	<section class="content-header">
	  ...
	  <ol class="breadcrumb">
		...
		<li><a href="#"><i class="fa fa-tree"></i>Ratings</a></li>
				 ...
				 <form method="post" name="form">	
				  ...
				  <input type="hidden" name="delId" id = "delId" value="" />	
					...
					if(mysql_num_rows($result)){
						...
						echo "<tr>
									<td><input type='checkbox' name='del[]' value='".$row["rating_id"]."'></td>
									<td>".$row["rating_name"]."</td>
									<td style='text-align:right;'>".$row["sortorder"]."</td>";?>
									<td>
										<img id="loading-image_<?php echo $row["rating_id"];?>" src="img/ajax-loader.gif" style="display:none;"/>
										<span id="ajaxStatus_<?php echo $row["rating_id"];?>">
											<a href="javascript:void(0);" onclick="changeContent(<?php echo $row['rating_id'];?> , <?php echo $row['isactive'];?> )">
											<?php echo $isactive; ?></a>
										</span>
									</td>
									<td>
										<a class='btn btn-primary' href='main.php?pg=addrating&id=<?=$row["rating_id"]?>'>Modify</a>
										<a class='btn btn-warning' onclick="return confirm('Are you sure want to delete this category?')" href='main.php?pg=viewrating&delId=<?=$row["rating_id"]?>' title="are you sure want to delete this category?">Delete</a>
									</td>
							 </tr>
					<?php			 
					...
					<div >
						...
						<button type="button" onclick="location.href='main.php?pg=addrating'"  name="submit_me"  class="btn btn-primary">Add</button>
					</div>
				  </form>
				</div>
				<!-- /.box-body -->
...
function changeContent(id,val){
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "phpajax/RatingActiveInactive.php", 
		data: data,
		beforeSend: function() {
