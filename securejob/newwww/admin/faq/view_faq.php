...
$Query= "SELECT * from tblfaqmaster order by sortorder asc";
...
if($delRqst && !isset($_REQUEST['del'])) {
	...
	if(is_numeric($delId) && $delId > 0 ){	
		...
		$where = " faq_id = {$delId} ";
		$db->Delete("tblfaqmaster",$where);		
	}
	$_SESSION['mt'] = "success";
	$_SESSION['me'] = "Question deleted successfully";
...
<div class="content-wrapper">
	...
	<section class="content-header">
	  ...
	  <ol class="breadcrumb">
		...
		<li><a href="#"><i class="fa fa-tree"></i>Faq's</a></li>
				 ...
				 <form method="post" name="form">	
				  ...
				  <input type="hidden" name="delId" id = "delId" value="" />	
					...
					if(mysql_num_rows($result)){
						...
						echo "<tr>
									<td><input type='checkbox' name='del[]' value='".$row["faq_id"]."'></td>
									<td>".$row["question"]."</td>
									<td>".substr($row["answer"],0,100)." ...</td>
									<td style='text-align:right;'>".$row["sortorder"]."</td>";?>
									<td>
										<img id="loading-image_<?php echo $row["faq_id"];?>" src="img/ajax-loader.gif" style="display:none;"/>
										<span id="ajaxStatus_<?php echo $row["faq_id"];?>">
											<a href="javascript:void(0);" onclick="changeContent(<?php echo $row['faq_id'];?> , <?php echo $row['isactive'];?> )">
											<?php echo $isactive; ?></a>
										</span>
									</td>
									<td>
										<a class='btn btn-primary' href='main.php?pg=addfaq&id=<?=$row["faq_id"]?>'>Modify</a>
										<a class='btn btn-warning' onclick="return confirm('Are you sure want to delete this faq?')" href='main.php?pg=viewfaq&delId=<?=$row["faq_id"]?>' title="are you sure want to delete this faq?">Delete</a>
									</td>
							 </tr>
					<?php			 
					...
					<div >
						...
						<button type="button" onclick="location.href='main.php?pg=addfaq'"  name="submit_me"  class="btn btn-primary">Add</button>
					</div>
				  </form>
				</div>
				<!-- /.box-body -->
...
function changeContent(id,val){
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "phpajax/FaqActiveInactive.php", 
		data: data,
		beforeSend: function() {
