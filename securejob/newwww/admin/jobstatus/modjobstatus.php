...
// Get data from db for modification 
	$a_id = $_REQUEST["id"];
	$sql = "select * from tbljobstatusmaster where id = {$a_id}"; 
	$result = $db->Query($sql);
	$a_name = "";
	list($id,$title,$status) = mysql_fetch_row($result);		
	$db->Free($result);
	$isActiveChecked = "";
	if($status == 1){ 
		 ...
		 <section class="content">
			  <div class="row">
					<div class="col-md-6">
					  ...
					  <div class="box box-primary">
						<div class="box-header with-border">
						  <h3 class="box-title">Modify Job Status</h3>
						</div>
						<div class="error"></div>
						<!-- /.box-header -->
						<!-- form start -->
						<form role="form" class="validateForm" name="Admin" action="main.php?pg=jobstatusproc" method="post" enctype="multipart/form-data" >
					...
					<input type="hidden" value="<?php echo $a_id;?>" name="id">
						...
						<input name="title" type="text" id="location" 
						class="form-control" value="<?php echo $title; ?>" maxlength="55" data-validation-engine="validate[required]"
						...
						<div class="form-group">
							...
							<div class="checkbox">
								<label><input type="checkbox" value="1" name="status" <?=$isActiveChecked?>  > NOTE:- Please tick this checkbox if you want to display this job status</label>
