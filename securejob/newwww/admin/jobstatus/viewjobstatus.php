...
// delete record
	...
	if($delRqst) {
		foreach ($_POST["del"] as $deletAdminID) {	
			if(is_numeric($deletAdminID) && $deletAdminID > 0 ){	
				...
				if($db->Delete("tbljobstatusmaster",$where)){
					$_SESSION['mt'] = "success";
		...
		header('Location:main.php?pg=viewjobstatus&delete');
		exit;
	...
	{
		$select_query = mysql_query("select * from tbljobstatusmaster where id=".$_REQUEST['id']."");
		$rows=mysql_num_rows($select_query);
		if($rows > 0)
		{
			$delete=mysql_query("delete from tbljobstatusmaster where id=".$_REQUEST['id']."");
			$res=mysql_fetch_array($select_query);
			$text=$res['name'];
			$_SESSION['mt'] = "success";
			$_SESSION['me'] = $text." job status has been deleted successfully.";
...
$select_query = mysql_query("select * from tbljobstatusmaster  ORDER By `title` ASC");	
?>
    ...
    <!-- Content Header (Page header) -->
		<section class="content-header">
		  ...
		  <ol class="breadcrumb">
			...
			<li><a href=""><i class="fa fa-location-arrow"></i>Job Status</a></li>
					 ...
					 <form method="post" name="form">	
					  ...
					  <input type="hidden" name="delId" id = "delId" value="" />	
						...
						if(mysql_num_rows($select_query)){
							...
							echo "<tr>
										...
										<td>".$row["title"]."</td>"; ?>
										 ...
										 <td><a class='btn btn-primary' href='main.php?pg=modjobstatus&id=<?=$row["id"]?>'>Modify</a>
										<a class='btn btn-warning' onclick="return confirm('Are you sure want to delete this job status?')" href='main.php?pg=viewjobstatus&id=<?=$row["id"]?>' title="are you sure want to delete this status?">Delete</a></td>
									 </tr>
						<?php			 
						...
						<div >
							...
							<button type="button" onclick="location.href='main.php?pg=addjobstatus'"  name="submit_me"  class="btn btn-primary">Add</button>
						</div>
					  </form>
					</div>
					<!-- /.box-body -->
...
function changeContent(id,val){
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "phpajax/JobStatusActiveInactive.php", 
		data: data,
		beforeSend: function() {
