...
if (!isAdminLoggedIn())
	...
	if((isset($_GET["id"])) && ($_GET["id"]!="")){
		$id = isset($_GET["id"])?($_GET["id"]):0;
		$sql = mysql_query("select * from tblbuild where id = ".$id."");
		
		list($id,$name,$status,$created_date) = mysql_fetch_row($sql);
		
		$sqlsettings = mysql_query("select * from tblbuild where id=$id");
	...
	if($_POST["act"] == 'add' || $_POST["act"] == 'edit')
	{
		...
		if($_POST["act"] == 'add' && $err==0){
			...
			if($db->Insert($data,"tblbuild")){
				$lastid=$db->LastInsert("tblbuild");
				$_SESSION['mt'] = "success";
				$_SESSION['me'] = "Build added successfully.";
		...
		if($_POST["act"] == 'edit' && $err==0){
			...
			$db->Update($data,"tblbuild","id=".$id."");					
			$_SESSION['mt'] = "success";
			$_SESSION['me'] = "Build updated successfully.";
			header("Location: main.php?pg=viewbuild");
			exit;
		}	
	}
	...
	<section class="content-header">
		<h1>Build</h1>
		  <ol class="breadcrumb">
			...
			<li><a href="<?php echo ADMIN_URL; ?>main.php?pg=viewbuild"><i class="fa fa-tags"></i>Build</a></li>
	...
	<section class="content">
		<div class="row">
			...
			<div class="checkbox">
				...
				?>
				<label><input type="checkbox" value="1" name="is_active" <?php echo $isActiveChecked;?> > NOTE:- Please tick this checkbox if you want to display this build</label>
			</div>				
