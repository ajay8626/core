...
if($act == "mod"){	
	if ( ($a_id != "")  && ($a_email != "") ){
		...
		if($totRows >0){	
			...
			$data = array('firstname'=>$a_name,'lastname'=>$a_lname,'phone'=>$phone,'email'=>$a_email,'address_1'=>$address_1,'address_2'=>$address_2,'address_3'=>$address_3,'postal_code'=>$postal_code,'status'=>$status,'modified_date'=>date('Y-m-d H:i:s'),'bank_name'=>$bank_name,'acc_holder_name'=>$acc_holder_name,'sort_code'=>$sort_code,'acc_number'=>$acc_number,'reg_no'=>$registration_no,'reg_vat_no'=>$vat_registration_no,'company_name'=>$company_name,'city_id'=>$city_id,'state_id'=>$state_id,'birthdate'=>$birthdate,'gender'=>$gender,'height'=>$height,'build'=>$build,'nationality'=>$nationality,'language'=>$language,'militry'=>$militry,'drive'=>$drive,'firstaid'=>$firstaid,'tattoos'=>$tattoos,'piercing'=>$piercing,'cscs_card'=>$cscs_card,'right_to_work_uk'=>$right_to_work_uk,'willing_to_travel'=>$willing_to_travel,'uk_driving_license'=>$uk_driving_license,'sia'=>$sia,'activity'=>$activity,'health'=>$health,'bio'=>$bio,'experience'=>$experience,'education_credentials'=>$education_credentials,'latitude'=>$mapLat,'longitude'=>$mapLong);
