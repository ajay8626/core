...
if (!isAdminLoggedIn())
	...
	if((isset($_GET["id"])) && ($_GET["id"]!="")){
		$id = isset($_GET["id"])?($_GET["id"]):0;
		$sql = mysql_query("select * from tbllanguage where id = ".$id."");
		
		list($id,$name,$status,$created_date) = mysql_fetch_row($sql);
		
		$sqlsettings = mysql_query("select * from tbllanguage where id=$id");
	...
	if($_POST["act"] == 'add' || $_POST["act"] == 'edit')
	{
		...
		if($_POST["act"] == 'add' && $err==0){
			...
			if($db->Insert($data,"tbllanguage")){
				$lastid=$db->LastInsert("tbllanguage");
				$_SESSION['mt'] = "success";
				$_SESSION['me'] = "Language added successfully.";
		...
		if($_POST["act"] == 'edit' && $err==0){
			...
			$db->Update($data,"tbllanguage","id=".$id."");					
			$_SESSION['mt'] = "success";
			$_SESSION['me'] = "Language updated successfully.";
			header("Location: main.php?pg=viewlanguage");
			exit;
		}	
	}
	...
	<section class="content-header">
		<h1>Language</h1>
		  <ol class="breadcrumb">
			...
			<li><a href="<?php echo ADMIN_URL; ?>main.php?pg=viewlanguage"><i class="fa fa-tags"></i>Language</a></li>
	...
	<section class="content">
		<div class="row">
			...
			<div class="checkbox">
				...
				?>
				<label><input type="checkbox" value="1" name="is_active" <?php echo $isActiveChecked;?> > NOTE:- Please tick this checkbox if you want to display this language</label>
			</div>				
