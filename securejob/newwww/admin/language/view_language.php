...
$Query= "SELECT * from tbllanguage order by name asc";
...
if($delRqst) {
	foreach ($_POST["del"] as $deletlanguageID) {	
		if(is_numeric($deletlanguageID) && $deletlanguageID > 0 ){	
			$where = "id  = {$deletlanguageID}";
			if($db->Delete("tbllanguage",$where)){
				$_SESSION['mt'] = "success";
				$_SESSION['me'] = "Language deleted successfully.";
			}else{
				$_SESSION['mt'] = "error";
				$_SESSION['me'] = "Error while delete Language. Please try again.";
				
			}
		}
		
	}
	header("location:main.php?pg=viewlanguage");
	exit;
...
<div class="content-wrapper">
	...
	<section class="content-header">
	  ...
	  <ol class="breadcrumb">
		...
		<li><a href="#"><i class="fa fa-tags"></i>Language</a></li>
				  ...
				  <input type="hidden" name="delId" id = "delId" value="" />	
					...
					if(mysql_num_rows($result)){
						...
						echo "<tr>
									...
									<td>
										<a class='btn btn-primary' href='main.php?pg=addlanguage&id=<?php echo $row["id"]?>'>Modify</a>
										<a class='btn btn-warning' onclick="return confirm('Are you sure want to delete this language?')" href='main.php?pg=viewlanguage&delId=<?php echo $row["id"]?>' title="are you sure want to delete this language?">Delete</a>
									</td>
							 </tr>
					<?php			 
					...
					<div >
						<!-- <button onclick="return confirm('Are you sure want to delete the selected language/languages?')" href='main.php?pg=viewlanguage' name="submit_me" id="delete"  class="btn btn-primary">Delete</button> -->
						<button type="button" onclick="confirmDelete()"  name="submit_me"  class="btn btn-primary">Delete</button>
						<button type="button" onclick="location.href='main.php?pg=addlanguage'"  name="submit_me"  class="btn btn-primary">Add</button>
					</div>
				  </form>
				</div>
				<!-- /.box-body -->
...
function changeContent(id,val){
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "phpajax/LanguageActiveInactive.php", 
		data: data,
		beforeSend: function() {
