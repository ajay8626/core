...
//$postcode = isset($rqst_data->postcode)?stripslashes($rqst_data->postcode):"";
	$sql="SELECT * FROM `tblappointments` where customer_id=".$customerId." and appointment_date > '".date("Y-m-d h:i:s")."' and appointment_status='confirmed'     $LastRecords order by appointment_date ASC LIMIT 0, $pageLimit";
	//echo $sql;
	//exit;
	$res=$db->Query($sql);
	...
	if($rows > 0)
	{
		while($result=mysql_fetch_assoc($res))
		{
			...
			$my_array[]=array("id"=>$result['id'],"customer_id"=>$result['customer_id'],
			"appointment_date"=>$result['appointment_date'],"time_slot"=>$timeslot,
			"no_windows"=>$result['no_windows'],"no_doors"=>$result['no_doors'],
			"no_conservatory"=>$result['no_conservatory'],"appointment_status"=>
			$appointment_status);
			$status_array=1;
