...
$Query= "SELECT * from tblwords order by name asc";
...
if($delRqst) {
	foreach ($_POST["del"] as $deletlanguageID) {	
		if(is_numeric($deletlanguageID) && $deletlanguageID > 0 ){	
			...
			if($db->Delete("tblwords",$where)){
				$_SESSION['mt'] = "success";
				$_SESSION['me'] = "Blocked Word deleted successfully.";
			}else{
				$_SESSION['mt'] = "error";
				$_SESSION['me'] = "Error while delete Word. Please try again.";
...
if($_REQUEST['del'])
{
	...
	{
		$where = " id = {$delId} ";
		$db->Delete("tblwords",$where);					
	}
	$_SESSION['mt'] = "success";
	$_SESSION['me'] = "Word deleted successfully";
	header("location:main.php?pg=viewwords");
	exit;
}
...
<div class="content-wrapper">
	...
	<section class="content-header">
	  ...
	  <ol class="breadcrumb">
		...
		<li><a href="#"><i class="fa fa-tags"></i>Words</a></li>
				  ...
				  <input type="hidden" name="delId" id = "delId" value="" />	
					...
					if(mysql_num_rows($result)){
						...
						echo "<tr>
									...
									<td>
										<a class='btn btn-primary' href='main.php?pg=addwords&id=<?php echo $row["id"]?>'>Modify</a>
										<a class='btn btn-warning' onclick="return confirm('Are you sure want to delete this Words?')" href='main.php?pg=viewwords&delId=<?php echo $row["id"]?>' title="are you sure want to delete this word?">Delete</a>
									</td>
							 </tr>
					<?php			 
					...
					<div >
						...
						<button type="button" onclick="location.href='main.php?pg=addwords'"  name="submit_me"  class="btn btn-primary">Add</button>
					</div>
				  </form>
				</div>
				<!-- /.box-body -->
...
function confirmDelete(){
	...
	if(f==0){
		alert("Atleast select one word to be deleted..!");
		return false;
	}
	else{
...
function changeContent(id,val){
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "phpajax/wordsActiveInactive.php", 
		data: data,
		beforeSend: function() {
