...
$Query= "SELECT * from tblnationality order by name asc";
...
if($delRqst) {
	if(is_numeric($delId) && $delId > 0 ){	
		$where = " id = {$delId} ";
		$db->Delete("tblnationality",$where);		
	}
	$_SESSION['mt'] = "success";
...
<div class="content-wrapper">
	...
	<section class="content-header">
	  ...
	  <ol class="breadcrumb">
		...
		<li><a href="#"><i class="fa fa-tags"></i>Nationality</a></li>
				  ...
				  <input type="hidden" name="delId" id = "delId" value="" />	
					...
					if(mysql_num_rows($result)){
						...
						echo "<tr>
									...
									<td>
										<a class='btn btn-primary' href='main.php?pg=addnationality&id=<?php echo $row["id"]?>'>Modify</a>
										<a class='btn btn-warning' onclick="return confirm('Are you sure want to delete this nationality?')" href='main.php?pg=viewnationality&delId=<?php echo $row["id"]?>' title="are you sure want to delete this nationality?">Delete</a>
									</td>
							 </tr>
					<?php			 
					...
					<div >
						...
						<button type="button" onclick="location.href='main.php?pg=addnationality'"  name="submit_me"  class="btn btn-primary">Add</button>
					</div>
				  </form>
				</div>
				<!-- /.box-body -->
...
function changeContent(id,val){
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "phpajax/NationalityActiveInactive.php", 
		data: data,
		beforeSend: function() {
