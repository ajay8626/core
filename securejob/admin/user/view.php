...
// delete record
	...
	if($delRqst) {
		foreach ($_POST["del"] as $deletAdminID) {	
			if(is_numeric($deletAdminID) && $deletAdminID > 0 ){	
				$where = "user_id  = {$deletAdminID}";
				if($db->Delete("tbluser",$where)){
					$_SESSION['mt'] = "success";
					$_SESSION['me'] = "User deleted successfully.";
				}else{
					$_SESSION['mt'] = "error";
		...
		header('Location:main.php?pg=viewuser&delete');
		exit;
	...
	{
		$select_query = mysql_query("select * from tbluser where user_id=".$_REQUEST['id']."");
		$rows=mysql_num_rows($select_query);
		if($rows > 0)
		{
			$delete=mysql_query("delete from tbluser where user_id=".$_REQUEST['id']."");
			$res=mysql_fetch_array($select_query);
			$text=$res['firstname']." ".$res['lastname'];
			$_SESSION['mt'] = "success";
			$_SESSION['me'] = $text." has been deleted successfully.";
		...
		{
			$_SESSION['mt'] = "error";
			$_SESSION['me'] = "Error while delete Customer. Please try again.";
			header('Location:main.php?pg=viewuser');
		    exit;		
		}
	}
$select_query = mysql_query("select * from tbluser ORDER By `user_id` DESC");	
?>
					 ...
					 <form method="post" name="form">	
					  ...
					  <input type="hidden" name="delId" id = "delId" value="" />	
						...
						if(mysql_num_rows($select_query)){
							...
							if(trim($row['verified_user']) == '1'){
                                $isVerified = "Verified";
                                $txtClass = "text-success bg-success";
                            } else{
                                $isVerified = "Unverified";
                                $txtClass = "text-danger bg-danger";
                            }
							
							echo "<tr>
										<td><input type='checkbox' name='del[]' value='".$row["user_id"]."'></td>
										<td>".$row["firstname"]."  ".$row["lastname"]."</td>
										<td>".$row["email"]."</td>
										<td style='text-align:right;'>".$row["phone"]."</td>
										<td >".$type."</td>";?>
										<td>
											<img id="loading-image_<?php echo $row["user_id"];?>" src="img/ajax-loader.gif" style="display:none;"/>
											<span id="ajaxStatus_<?php echo $row["user_id"];?>">
												<a href="javascript:void(0);" onclick="changeContent(<?php echo $row['user_id'];?> , <?php echo $row['status'];?> )">
												<?php echo $isactive; ?></a></span>
                                        </td>
										<td class="<?php echo $txtClass; ?>"><?php echo $isVerified; ?></td>
										 <td><a class='btn btn-primary' href='main.php?pg=moduser&id=<?=$row["user_id"]?>'>Modify</a>
										<a class='btn btn-warning' onclick="return confirm('Are you sure want to delete this user?')" href='main.php?pg=viewuser&id=<?=$row["user_id"]?>' title="are you sure want to delete this user?">Delete</a></td>
									 </tr>
						<?php			 
						...
						<div >
							...
							<button type="button" onclick="location.href='main.php?pg=adduser'"  name="submit_me"  class="btn btn-primary">Add</button>
						</div>
					  </form>
					</div>
					<!-- /.box-body -->
