...
// delete record
	...
	if($delRqst) {
		foreach ($_POST["del"] as $deletAdminID) {	
			if(is_numeric($deletAdminID) && $deletAdminID > 0 ){	
				...
				$where2 = "location_id  = {$deletAdminID}";
				if($db->Delete("tblstates",$where)){
					$db->Delete("tbllocation",$where2);
					$_SESSION['mt'] = "success";
					$_SESSION['me'] = "County deleted successfully.";
				}else{
					$_SESSION['mt'] = "error";
					$_SESSION['me'] = "Error while delete County. Please try again.";
	...
	if(isset($_REQUEST['id']) && $_REQUEST['id']!='')
	{
		...
		if($rows > 0)
		{
			...
			$deletes=mysql_query("delete from tbllocation where location_id=".$_REQUEST['id']."");
			$res=mysql_fetch_array($select_query);
			$text=$res['name'];
			$_SESSION['mt'] = "success";
			$_SESSION['me'] = $text." county has been deleted successfully.";
    ...
    <!-- Content Header (Page header) -->
		<section class="content-header">
		  ...
		  <ol class="breadcrumb">
			...
			<li><a href=""><i class="fa fa-location-arrow"></i>County</a></li>
