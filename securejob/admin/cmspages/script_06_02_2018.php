...
require_once("../../config.php");
	...
	if($sOrder=='')
	{
		$sOrder=" order by tblcmspages.page_id desc";
	}		
	...
	$sQuery1 = "SELECT tblcmspages.page_id FROM tblcmspages where 1 $sWhere GROUP BY tblcmspages.page_id ORDER BY tblcmspages.page_id";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		$status=$aRow['is_active'];
			...
			$row[] = '<a href='.ADMIN_URL.'main.php?pg=addcms&id='.$id.' class="btn btn-primary">Modify</a>';
								/* '<a href='.ADMIN_URL.'main.php?pg=viewcms&delId='.$id.' onclick="'.$confirm.'" class="btn delbtn confirm" title="Are you want to delete this page?"> Delete </a>'; */
