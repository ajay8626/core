...
if (!isAdminLoggedIn())
	...
	if((isset($_GET["id"])) && ($_GET["id"]!="")){
		$id = isset($_GET["id"])?($_GET["id"]):0;
		$sql = mysql_query("select * from tblcoursecategory where category_id = ".$id."");
	...
	if($_POST["act"] == 'add' || $_POST["act"] == 'edit')
	{
		...
		if($id == 0 && $err==0)
		{
			$sql_category = "select * from tblcoursecategory WHERE category_name = '$name'";
		...
		if($_POST["act"] == 'add' && $err==0){
			...
			if($db->Insert($data,"tblcoursecategory")){
				$lastid=$db->LastInsert("tblcoursecategory");
				$_SESSION['mt'] = "success";
		...
		if($_POST["act"] == 'edit' && $err==0){
			...
			$db->Update($data,"tblcoursecategory"," category_id=".$id."");					
			$_SESSION['mt'] = "success";
	...
	<section class="content-header">
		<h1>Course Categories</h1>
		  <ol class="breadcrumb">
			...
			<li><a href="<?php echo ADMIN_URL; ?>main.php?pg=viewcoursecategories"><i class="fa fa-tree"></i>Course Categories</a></li>
	...
	<section class="content">
		<div class="row">
			...
			<div class="form-group">
				...
				<?php if($sortorder == ''){ $sortorder = getSortOrderCategory('tblcoursecategory') + 1; } ?>
