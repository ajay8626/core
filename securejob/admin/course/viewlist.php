...
// delete record
	...
	if($delRqst && isset($_REQUEST['del'])) {
		...
		foreach ($_POST["del"] as $deletAdminID) {	
			if(is_numeric($deletAdminID) && $deletAdminID > 0 ){	
				...
				if($db->Delete("tblcoursemaster",$where)){
					$_SESSION['mt'] = "success";
	...
	if(isset($_REQUEST['id']) && $_REQUEST['id']!='')
	{
		...
		$select_query = mysql_query("select * from tblcoursemaster where course_id=".$_REQUEST['id']."");
		$rows=mysql_num_rows($select_query);
		if($rows > 0)
		{
			$delete=mysql_query("delete from tblcoursemaster where course_id=".$_REQUEST['id']."");
			$_SESSION['mt'] = "success";
...
$select_query = mysql_query("SELECT `tblcoursemaster`.`course_id`,`tblcoursemaster`.`course_title`, `tblcoursemaster`.`location`, `tblcoursemaster`.`price`,`tblcoursemaster`.`status`,`tbluser`.`firstname`,`tbluser`.`lastname`
    ...
    <!-- Content Header (Page header) -->
		<section class="content-header">
		  ...
		  <ol class="breadcrumb">
			...
			<li><a href="#"><i class="fa fa-suitcase"></i>Courses</a></li>
						...
						if(mysql_num_rows($select_query)){
							...
							echo "<tr>
										...
										<td>
										 ...
										 <td><a class='btn btn-primary' href='main.php?pg=viewcoursedetail&id=<?=$row["course_id"]?>'>View</a>
										<a class='btn btn-warning' onclick="return confirm('Are you sure want to delete this course?')" href='main.php?pg=deletecourse&id=<?=$row["course_id"]?>' title="are you sure want to delete this job?">Delete</a></td>
									 </tr>
						<?php			 
