...
$start_date = 	isset($_REQUEST["start_date"])&&$_REQUEST["start_date"]!=''?date('Y-m-d',strtotime($_REQUEST["start_date"])):'';
...
if($act == "add"){	
	 ...
	 if($jobname != "" || $price != '' || $description != ''){
			...
			$data = array('name'=>$jobname,'description'=>$description,'price'=>$price,'image'=>$job_image,'course_days'=>$days,'course_hours'=>$hours,'start_date'=>$start_date,'latitude'=>$latitude,'longitude'=>$longitude,'address1'=>$address_1,'address2'=>$address_2,'postalcode'=>$postal_code,'country_id'=>$country_id,'state_id'=>$state_id,'city_id'=>$city_id,'status'=>$status,'isfeatured'=>$is_featured,'course_time'=>$course_time);
...
if($act == "mod"){	
	if($jobname != "" || $price != '' || $description != ''){
		...
		$data = array('name'=>$jobname,'description'=>$description,'price'=>$price,'course_days'=>$days,'course_hours'=>$hours,'start_date'=>$start_date,'latitude'=>$latitude,'longitude'=>$longitude,'address1'=>$address_1,'address2'=>$address_2,'postalcode'=>$postal_code,'country_id'=>$country_id,'state_id'=>$state_id,'city_id'=>$city_id,'status'=>$status,'isfeatured'=>$is_featured,'course_time'=>$course_time);
