...
$Query= "SELECT * from tblcouponcodemaster order by couponcode asc";
...
if($delRqst && !isset($_REQUEST['del'])) {
	...
	if(is_numeric($delId) && $delId > 0 ){	
		...
		$where = " coupon_id = {$delId} ";
		$db->Delete("tblcouponcodemaster",$where);		
	}
	$_SESSION['mt'] = "success";
	$_SESSION['me'] = "Coupon deleted successfully";
...
<div class="content-wrapper">
	...
	<section class="content-header">
	  ...
	  <ol class="breadcrumb">
		...
		<li><a href="#"><i class="fa fa-tree"></i>Coupon Master</a></li>
				 ...
				 <form method="post" name="form">	
				  ...
				  <input type="hidden" name="delId" id = "delId" value="" />	
					...
					<tr>
					  ...
					  <th>Coupon Code</th>
					  <th>Coupon Type</th>
					  <th style="text-align:right;">Coupon Amount</th>
					  <th>Status</th>
					  <th>Action</th>
					</tr>
					</thead>
					...
					if(mysql_num_rows($result)){
						...
						echo "<tr>
									<td><input type='checkbox' name='del[]' value='".$row["coupon_id"]."'></td>
									<td>".$row["couponcode"]."</td>
									<td>".$type."</td>
									<td style='text-align:right;'>".number_format($row["amount"],2).$add."</td>";?>
									<td>
										<img id="loading-image_<?php echo $row["coupon_id"];?>" src="img/ajax-loader.gif" style="display:none;"/>
										<span id="ajaxStatus_<?php echo $row["coupon_id"];?>">
											<a href="javascript:void(0);" onclick="changeContent(<?php echo $row['coupon_id'];?> , <?php echo $row['status'];?> )">
											<?php echo $isactive; ?></a>
										</span>
									</td>
									<td>
										<a class='btn btn-primary' href='main.php?pg=addcoupon&id=<?=$row["coupon_id"]?>'>Modify</a>
										<a class='btn btn-warning' onclick="return confirm('Are you sure want to delete this coupon?')" href='main.php?pg=viewcoupon&delId=<?=$row["coupon_id"]?>' title="are you sure want to delete this category?">Delete</a>
									</td>
							 </tr>
					<?php			 
					...
					<div >
						...
						<button type="button" onclick="location.href='main.php?pg=addcoupon'"  name="submit_me"  class="btn btn-primary">Add</button>
					</div>
				  </form>
				</div>
				<!-- /.box-body -->
...
function changeContent(id,val){
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "phpajax/CouponActiveInactive.php", 
		data: data,
		beforeSend: function() {
