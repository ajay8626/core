...
// delete record
	...
	if($delRqst) {
		foreach ($_POST["del"] as $deletAdminID) {	
			if(is_numeric($deletAdminID) && $deletAdminID > 0 ){	
				$where = "job_id  = {$deletAdminID}";
				if($db->Delete("tbljobs",$where)){
					$_SESSION['mt'] = "success";
					$_SESSION['me'] = "Jobs deleted successfully.";
				}else{
					$_SESSION['mt'] = "error";
					$_SESSION['me'] = "Error while delete Jobs. Please try again.";
					
				}
			}
			
		}
		header('Location:main.php?pg=viewjobs&delete');
		exit;
	...
	{
		$select_query = mysql_query("select * from tbljobs where job_id=".$_REQUEST['id']."");
		$rows=mysql_num_rows($select_query);
		if($rows > 0)
		{
			$delete=mysql_query("delete from tbljobs where job_id=".$_REQUEST['id']."");
			$_SESSION['mt'] = "success";
			$_SESSION['me'] = "Job has been deleted successfully.";
		...
		{
			$_SESSION['mt'] = "error";
			$_SESSION['me'] = "Error while delete job. Please try again.";
			header('Location:main.php?pg=viewjobs');
		    exit;		
		}
	}
$select_query = mysql_query("select user.*,job.* from tbljobs as job left join tbluser as user on user.user_id = job.job_user_id ORDER By job.job_id DESC");	
?>
					 ...
					 <form method="post" name="form">	
					  ...
					  <input type="hidden" name="delId" id = "delId" value="" />	
						...
						if(mysql_num_rows($select_query)){
							...
							echo "<tr>
										<td><input type='checkbox' name='del[]' value='".$row["job_id"]."'></td>
										<td>".$row["job_name"]."</td>
										<td>".$row['firstname'].' '.$row['lastname']."</td>
										<td>".$row['job_location']."</td>
										<td style='text-align:right;'>&pound;".$row["price"]."</td>";?>
										<td style='text-align:right;'><?php echo "<a href='main.php?pg=viewjobbids&id=".$row["job_id"]."' target='_blank'>".bidsCount($row["job_id"])."</a>"; ?></td>
										<td>
											<img id="loading-image_<?php echo $row["job_id"];?>" src="img/ajax-loader.gif" style="display:none;"/>
											<span id="ajaxStatus_<?php echo $row["job_id"];?>">
												<a href="javascript:void(0);" onclick="changeJobs(<?php echo $row['job_id'];?> , <?php echo $row['status'];?> )">
												<?php echo $isactive; ?></a></span></td>
										
										 <td><a class='btn btn-primary' href='main.php?pg=modjob&id=<?=$row["job_id"]?>'>Modify</a>
										<a class='btn btn-warning' onclick="return confirm('Are you sure want to delete this job?')" href='main.php?pg=viewjobs&id=<?=$row["job_id"]?>' title="are you sure want to delete this job?">Delete</a>&nbsp;<a class='btn btn-primary' href='main.php?pg=viewjobbids&id=<?=$row["job_id"]?>'>View Bids</a></td>
									 </tr>
						<?php			 
...
function changeJobs(id,val){
	...
	$.ajax({
		type: "POST",
		dataType: "json",
		url: "phpajax/JobsActiveInactive.php", 
		data: data,
		beforeSend: function() {
