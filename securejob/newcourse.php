...
jQuery(document).ready(function($) {	
	...
	$('#result').html('');
	load_more(CateId,getCookie('course'),activesort);		
	});
...
//alert(cateid);
load_more(CateId,pagenumber,sort);
$(window).scroll(function(){
	...
	if($('#last_page').val()==0)
        ...
        load_more(CateId,getCookie('course'),activesort); //load content   
		    }
		 }
 });    
function load_more(CateId,page,sort){	
	...
	if($('#last_page').val() != 1) {	
      $.ajax({
			url: 'newget_course.php?cateid='+CateId+'&page=' +page+'&sort='+sort+searchval,
			type: "get",
			datatype: "html",
			beforeSend: function()
			{
		...
		.done(function(data)
		{
			if(data.length == 0){
				...
				if ( $( "#result li h3.nc" ).length ==0 && $( "#result li div.crs_list_lft" ).length <= 0 ) {					
					$("#result").append('<li><h3 class="nc">No Course Available.</h3></li>');
				}
				return;
			}
...
/*if ($CateId != "" && isset($CateId)) {
	...
	//$cat_id = $_GET['id'];
	$sql = "SELECT * from tblcoursemaster where category_id = $CateId AND status=1";
			...
			<div class="col-xs-12 col-md-8 stj_crs_dv">
				...
				<div class="stj_crs_inn">
				  <div class="stj_sort">
						...
						<div class="srch_dv">
							<input class="txt_srch" name="srh_newjobs" id="srh_newjobs" placeholder="Search" type="text" value="<?php echo $search_val;?>">
