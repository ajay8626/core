...
/*function load_more(cateid,pagenumber,sort){
	...
	//window.location = 'newget_course.php?cateid='+cateid+'&page=' +page+'&sort='+sort+searchval+'&cateid='+cateid;
	
	if($('#last_page').val() != 1) {
      $.ajax({
			url: 'newget_course.php?cateid='+cateid+'&page=' +page+'&sort='+sort+searchval+'&cateid=5',
			type: "get",
			datatype: "html",
			beforeSend: function()
			{
...
/*select cource category(changes)*/
$sql = "SELECT * from tblcategory where isactive = 1 order by category_name asc";
$result = $db->Query($sql);
                    			...
                    			<div class="stj_ad_srch_inner">
                    				<ul>
										...
										<li>
											...
											<select name="category_id" id="category_id" data-validation-engine="validate[required]" data-errormessage-value-missing="Please select category" onchange="getCities(this.value)">
                  ...
                  <div class="crs_listing">
                  	...
                  	<ul>
                  		...
                  		while ($row = mysql_fetch_assoc($result)) { 
							...
							if($row['image']!='')
							{
								$image='<img src='.CATEGORY_IMG_URL.$row['image'].' width="100px">';
							}else{
								$image='<img src='.CATEGORY_IMG_URL."default.png".' width="100px">';
							}

                  			?>
						    <li>
                  			<a href="<?php echo SITE_URL.'newcourse.php?catid='.$row["category_id"]; ?>">
                  				<div class="imgwrap">
									<?php echo $image; ?>								
								</div>
...
<script>
    ...
    function searchData(latitude,longitude) {
        ...
        $.ajax({
			type: 'GET',
			url: 'newget_courses_advance.php?courseTitle='+courseTitle+'&courseDuration='+courseDuration+'&courseLocation='+courseLocation+'&longi='+longitude+'&lati='+latitude+'&zipMiles='+zipMiles+'&coursePrice='+coursePrice+'&startDate='+startDate+'&endDate='+endDate,
			datatype: "html",
			beforeSend: function()
			{
				$('.stj_loader').html("Loading...");
