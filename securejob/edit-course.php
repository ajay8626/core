...
if(isset($_POST['submit']) && $_POST['submit']!='' && !empty($_REQUEST['id']))
{
	...
	//print_r($ins_array);exit;
	//$db->Insert($ins_array, 'tblcoursemaster');
	$datawhere ="course_id ={$course_id}";
	$db->Update($ins_array,"tblcoursemaster",$datawhere);
...
$course_id = $_REQUEST["id"];
$sql = "select * from tblcoursemaster where course_id = {$course_id}"; 
$result = $db->Query($sql);
$totRows = mysql_num_rows($db->Query($sql));
if ($totRows > 0) {
	$a_name = "";
	list($course_id,$course_title,$course_body,$category_id,$start_date,$end_date,$start_time,$end_time,$latitude,$longitude,$postalcode,$duration,$specify_days,$price,$enrollment_limit,$description,$image,$location,$course_body_certificate,$declaration,$created_date,$created_by,$modified_date,$modified_by,$status) = mysql_fetch_row($result);		
	$db->Free($result);
	$isActiveChecked = "";
	if($status == 1){ 
...
<?php
/*echo "<pre>course_id -> ";print_r($course_id);
echo "<br>course_title -> ";print_r($course_title);
echo "<br>course_body -> ";print_r($course_body);
echo "<br>category_id -> ";print_r($category_id);
echo "<br>start_date -> ";print_r($start_date);
echo "<br>start_time -> ";print_r($start_time);
echo "<br>latitude -> ";print_r($latitude);
echo "<br>longitude -> ";print_r($longitude);
echo "<br>postalcode -> ";print_r($postalcode);
echo "<br>duration -> ";print_r($duration);
echo "<br>specify_days -> ";print_r($specify_days);
echo "<br>price -> ";print_r($price);
echo "<br>enrollment_limit -> ";print_r($enrollment_limit);
echo "<br>description -> ";print_r($description);
echo "<br>image -> ";print_r($image);
echo "<br>location -> ";print_r($location);
echo "<br>course_body_certificate -> ";print_r($course_body_certificate);
echo "<br>declaration -> ";print_r($declaration);
echo "<br>created_date -> ";print_r($created_date);
echo "<br>created_by -> ";print_r($created_by);
echo "<br>modified_date -> ";print_r($modified_date);
echo "<br>modified_by -> ";print_r($modified_by);
echo "<br>status -> ";print_r($status);
exit;*/
?>
...
<title>Edit Course - SECURE THAT JOB</title>
...
function removeLicense(certiId)
{
	var certiId = certiId;
	var confirm2Remove = confirm("Are you sure you want to remove course image?");
...
function removeCompanyCerti(certiId)
{
	var certiId = certiId;
	var confirm2Remove = confirm("Are you sure you want to remove company certificate?");
		...
		<div class="row">
			<div class="col-xs-12 col-md-12 stj_about_inn">
				<div class="stj_abt_con min_content jobdetail">
					<form id="course_form"  method="post" name="edit-profile" enctype="multipart/form-data" class="validateForm" action="">
					<div class="stj_pb_edit">
						...
						<div class="stj_pb_frm">
							...
							<ul>
							    <li>
							     <label>Recognised Course Body<em>*</em></label>
							      <input class="txt_lg" name="course_body" id="course_body" data-validation-engine="validate[required]" data-errormessage-value-missing="Please enter course body" value="<?php echo $course_body; ?>" type="text">
						        </li>
								<li class="clr_lft">
								     <label>Course Title <em>*</em></label>
							         <input class="txt_lg" name="course_title" id="course_title" data-validation-engine="validate[required]" data-errormessage-value-missing="Please enter course title" value="<?php echo $course_title; ?>" type="text">
								</li>
								
								<li class="clr_lft">
									<div class="start_date" style="float: left;width: 50%;">
								     	<label>Course Start Date <em>*</em></label>
							         	<input class="txt_lg datepicker" name="start_date" style="width:200px;" data-validation-engine="validate[required]" data-errormessage-value-missing="Please select Date of Birth" value="<?php echo date('d/m/Y', strtotime($start_date));  ?>" id="datepicker" type="text">
							         </div>
							         <div class="clockpicker" data-autoclose="true" style="float: right;width: 47%;">
							         	<label>Time <em>*</em></label>
							         	<input class="txt_lg" name="start_time" style="width:200px;" data-validation-engine="validate[required]" data-errormessage-value-missing="Please select time" value="<?php echo $start_time;  ?>" id="start_time" type="text">
							         </div>
							    </li>
							    <li>
							         <div class="end_date" style="float: left;width: 50%;">
								     	<label>Course End Date <em>*</em></label>
							         	<input class="txt_lg datepicker1" name="end_date" data-validation-engine="validate[required]" data-errormessage-value-missing="Please select Date of Birth" value="<?php echo date('d/m/Y', strtotime($end_date));  ?>" id="datepicker1" type="text" autocomplete="off">
							         </div>
							         <div class="clockpicker" data-autoclose="true" style="float: right;width: 47%;">
							         	<label>Course End Time <em>*</em></label>
							         	<input class="txt_lg" name="end_time" data-validation-engine="validate[required]" data-errormessage-value-missing="Please select time" value="<?php echo $end_time;  ?>" id="end_time" type="text" autocomplete="off">
								...
								<input class="txt_lg" name="duration"  id="duration"  data-validation-engine="validate[required]" data-errormessage-value-missing="Please enter duration" value="<?php echo $duration;?>" type="text">
						        </li>
								<li class="clr_lft">
								     <label>Specify Days (eg. Monday - Friday) <em>*</em></label>
							         <input class="txt_lg" name="specify_days" id="specify_days" data-validation-engine="validate[required]" data-errormessage-value-missing="Please enter course title" value="<?php echo $specify_days; ?>" type="text">
								</li>
								<li >
								     <label>Amount (&#163;)<em>*</em></label>
							         <input class="txt_lg" value="<?php echo $price; ?>" name="price" maxlength="12" data-validation-engine="validate[required]" data-errormessage-value-missing="Please enter price" type="tel">
								</li>
								<li >
								     <label>Enrolment Limit <em>*</em></label>
							         <input class="txt_lg" value="<?php echo $enrollment_limit; ?>" name="enrolment_limit" maxlength="12" data-validation-engine="validate[required]" data-errormessage-value-missing="Please enter limit" type="tel">
								</li>
								<li class="clr_lft">
								     <label>Description (Please add any additional details e.g. 9am - 6pm with a one hour break)<em>*</em></label>
							         <textarea name="description" id="description" data-validation-engine="validate[required]" data-errormessage-value-missing="Please enter description"><?php echo $description; ?></textarea>
								</li>
								...
								<li class="clr_lft">
									...
									<div class="file-upload">
										<div class="file-select">   
											...
											<input type="file" name="image" multiple name="license[]" data-validation-engine="validate[funcCall[geThanLicense[]]]"  id="chooseLicense">
										</div> 
									</div>
									...
									<?php if($licenseMime != 'pdf'){ ?>
										<span id="edit_image_pre">
											<a class="fancybox" rel="group" href="<?php echo COURSE_NEWIMG_URL.$image; ?>">
												<img src="<?php echo COURSE_NEWIMG_URL.$image; ?>" style="width:100px;" alt="">
											</a>
											<input type="button" name="remove_certi" class="btn btn-srch-pc remove_certi" value="Remove" onclick="javascript:return removeLicense(<?php echo $course_id ?>);">
										</span>
									<?php }else{ ?>
										<span id="edit_image_pre">
											<a class="fancybox_pdf" rel="group" href="<?php echo COURSE_NEWIMG_URL.$image; ?>">
							    ...
							    <li>
							         ...
							         <input class="txt_lg" name="postal_code" id="postal_code" data-validation-engine="validate[required]" data-errormessage-value-missing="Please enter course title" value="<?php echo $postalcode; ?>" type="text">
								</li>
								...
								<li class="clr_lft">
									...
									<?php 
									if (!empty($course_body_certificate)) { 
									?>
									
									<?php if($companyCertiMime != 'pdf'){ ?>
										<span id="edit_certi_pre">
											<a class="fancybox" rel="group" href="<?php echo COURSE_CERTI_IMG_URL.$course_body_certificate; ?>">
												<img src="<?php echo COURSE_CERTI_IMG_URL.$course_body_certificate; ?>" style="width:100px;" alt="">
											</a>
											<input type="button" name="remove_certi" class="btn btn-srch-pc remove_certi" value="Remove" onclick="javascript:return removeCompanyCerti(<?php echo $course_id ?>);">
										</span>
									<?php }else{ ?>
										<span id="edit_certi_pre">
											<a class="fancybox_pdf" rel="group" href="<?php echo COURSE_CERTI_IMG_URL.$course_body_certificate; ?>">
												<img src="images/pdf_image.png" style="width:100px;" alt="">
											</a>
											<input type="button" name="remove_certi" class="btn btn-srch-pc remove_certi" value="Remove" onclick="javascript:return removeCompanyCerti(<?php echo $course_id ?>);">
										</span>
								...
								<li class="input_declare">
								     <label><input type="checkbox" id="declaration" name="declaration" value="1" data-validation-engine="validate[required]"> I declare that I am posting a legitimate course </label>
								</li>
							...
							<?php } ?>
							<div class="nextbtn">
				            	<input value="Save" name="submit" id="save" class="btn_lg" type="submit">				
				            </div>
...
},'Course end date must be greater than start date1');
